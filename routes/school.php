...
use App\Http\Controllers\School\ResourcesController;
...
Route::middleware(['school_auth'])->group(function () {
    Route::middleware(['check_session'])->group(function () {
        ...
        Route::controller(AssignPeriodController::class)->as('school.')->group(function () {
            ...
            Route::get('/time-table/assign-periods/get-subject-by-teacher/{id}', 'getSectionByTeacher')->name('timetable.assign_periods.get-subject-by-class');
            ...
            Route::get('/time-table/teachers', 'teaherTimeTable')->name('timetable.teacher-timetable');
        });
        ...
        Route::controller(StudentController::class)->as('school.')->group(function () {
            ...
            Route::get('/hs_students', 'hs')->name('hs_students');
            Route::get('/pharmacy_students', 'pharmacy')->name('pharmacy_students');
            ...
            Route::get('/download_student_detail/{id}','download_detail_pdf')->name('students.pdf');
        });
        ...
        Route::controller(FeesController::class)->as('school.')->group(function () {
            Route::get('/fees/fee-structure','index')->name('fees.fee-structure');
            Route::get('/fees/fee-structure/create','create')->name('fees.fee-structure.create');
            Route::post('/fees/fee-structure/store','store')->name('fees.fee-structure.store');
            Route::get('/fees/fee-structure/{id}/edit','edit')->name('fees.fee-structure.edit');
            Route::post('/fees/fee-structure/update','update')->name('fees.fee-structure.update');
            Route::post('/fees/fee-structure/delete','delete')->name('fees.fee-structure.delete');
            // In your routes/web.php
            Route::get('/get-classes-by-year','getClassesByYear')->name('fees.getClassesByYear');
            Route::get('/school/fees/getStudentsByClass', 'getStudentsByClass')->name('fees.getStudentsByClass');
            Route::get('/school/fees/getStudentdueAmount', 'getStudentdueAmount')->name('fees.getStudentdueAmount');

            //FeePayment
            Route::get('/fees/fee-payment','fee_payment_index')->name('fees.fee-payment');
            Route::get('/fees/fee-payment/{student_id}/create','fee_payment_create')->name('fees.fee-payment.create');
            Route::get('/fees/admin-fee-payment/create','admin_fee_payment_create')->name('fees.admin-fee-payment.create');
            Route::post('/fees/fee-payment/store','fee_payment_store')->name('fees.fee-payment.store');
            Route::post('/fees/admin-fee-payment/store','admin_fee_payment_store')->name('fees.admin-fee-payment-store');
            Route::get('/fees/admin-fee-payment/delete/{id}','admin_fee_payment_delete')->name('fees.admin-fee-payment-delete');
            Route::get('/get-students-by-class-id/{id}','getStudentsByClassId');
            Route::get('/fees/fee-payment/{student_id}/detail','fee_payment_detail')->name('fees.fee-payment.detail');
            Route::get('/fees/fee-payment/{id}/edit','fee_payment_edit')->name('fees.fee-payment.edit');
            Route::post('/fees/fee-payment/specific/update','fee_payment_specific_update')->name('fees.fee-payment.specific.update');
            Route::post('/fees/fee-payment/specific/delete','fee_payment_specific_delete')->name('fees.fee-payment.specific.delete');
            Route::post('/fees/fee-payment/delete','fee_payment_delete')->name('fees.fee-payment.delete');
            Route::get('/fees/admin-fee-payment/invoice/{id}','admin_fee_payment_invoice')->name('fees.admin-fee-payment-invoice');

            //fee payment history

            Route::get('/fees-history/fee-payment-history','fee_payment_history_index')->name('fees.fee-payment-history');
            Route::get('/fees-history/fee-payment-history-detail/{id}','fee_payment_history_detail')->name('fees.fees-history.fee-payment-history-detail');

            Route::get('/refferal-data','fee_payment_refferal_data')->name('fees.refferal-data');
        });
        ...
        Route::controller(PushNotificationController::class)->as('school.')->group(function () {
            ...
            Route::get('/notifications/send/{id}','send_notification')->name('send-notification');
        });
        ...
        Route::controller(ExamTimeSheetController::class)->as('school.')->group(function () {
            ...
            Route::get('/exams/exam-timetable/get-exam-date/{id}','getExamDate');
        });
        ...
        Route::controller(AttendanceController::class)->as('school.')->group(function () {
            ...
            Route::post('/leave-applications/accept','acceptLeaveApplication')->name('leave-applications.accept');
            Route::post('/leave-applications/rejected','rejectedLeaveApplication')->name('leave-applications.rejected');

            Route::get('/student-leave-applications','viewStudentLeaveApplication')->name('student-leave-applications');
            Route::post('/student-leave-applications/accept','acceptStudentLeaveApplication')->name('student-leave-applications.accept');
            Route::post('/student-leave-applications/rejected','rejectedStudentLeaveApplication')->name('student-leave-applications.rejected');
        });


        Route::controller(ResourcesController::class)->as('school.')->group(function () {
            Route::get('/resources/home-work', 'homeWork')->name('resources.home-work');
            Route::get('/resources/home-work/{id}/detail', 'homeWorkDetail')->name('resources.home-work.detail');
        });
    });

    Route::controller(SessionController::class)->as('school.')->group(function () {
        Route::get('/view-all-session','viewAllSession')->name('view-all-session');
        Route::get('/view-all-session/create', 'create')->name('view-all-session.create');
        Route::post('/view-all-session/store','store')->name('view-all-session.store');
        Route::get('/view-all-session/{id}/edit', 'edit')->name('view-all-session.edit');
        Route::post('/view-all-session/update','update')->name('view-all-session.update');
        Route::post('/view-all-session/delete','delete')->name('view-all-session.delete');
        Route::post('/view-all-session/block','block')->name('view-all-session.block');


        Route::get('/migrations','migrations')->name('migrations');
        Route::post('/migrations/store','migrationsStore')->name('migrations.store');
    });
