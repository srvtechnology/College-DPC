...
class AssignPeriodController extends Controller
{
    ...
    public function edit($id)
    {
        ...
        $getSectionByTeacher = $this->getSectionByTeacher($assignPeriod->staff_id);
        $subjects = $getSectionByTeacher;
    ...
    public function getSectionByTeacher($id)
    {
        $school = getSchoolInfoByUsername(Auth::user()->username);
        $staff = Staff::find($id);
        ...
        {
            foreach($staff->subjects as $sub_index => $subject)
            {
                $subjects[$sub_index] = [
                    'id' => $subject->subject->id,
                    'name' => $subject->subject->name,
    ...
    public function viewTimeTable()
    {
        ...
        foreach($assign_periods as $assign_period)
        {
            ...
            $periods[] = [
                'class' => $assign_period->class->name,
                'section' => $assign_period->section->name,
    ...
    public function teaherTimeTable()
    {
        $school = getSchoolInfoByUsername(Auth::user()->username);
        $teachers = Staff::where(['school_id' => $school->id,'is_deleted' => '0'])->get();
        ...
        if(!empty($staff_id))
        {
            ...
            $assign_periods = TimeTableAssignPeriod::where('school_id',$school->id)->where('staff_id',$staff_id)->get();
            if(count($assign_periods) == 0)
            {
                return back()->with('error','No Record Found');
            }
        ...
        return view("school.assign-periods.teacher_timetable")->with(compact('school','teachers','start_time','weekdays','periods'));
    }
}
