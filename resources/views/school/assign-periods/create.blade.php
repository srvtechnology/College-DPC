...
@section('content')
    ...
    @push('footer-script')
        <script>
            ...
            $(".teacher_id1").on("change", function(){
                var teacher_id = $(this).val();
                getSubjectByTeacher(teacher_id, 1);
            });
            ...
            function getSubjectByTeacher(teacher_id,counter){
                $.ajax({
                    url: '{{ url('school/time-table/assign-periods/get-subject-by-teacher') }}' + '/' + teacher_id,
                    type: 'GET',
                    success: function(response) {
                        $("#subject_id"+counter).html('');
                        $(response).each(function(index, element) {
            ...
            function getSubjectByTeacherEvent(event) {
                var classList = event.target.classList;
                var className = "";

                for (var i = 0; i < classList.length; i++) {
                  if (classList[i].startsWith("teacher")) {
                    className = classList[i];
                    break;
                  }
                }
                var counter = className.replace(/^\D+/g, '');
                var class_id = $("."+className).val();
                getSubjectByTeacher(class_id, counter);
            }
                    ...
                    <div class="col-md-3 mb-3">
                        <div class="form-group">
                            ...
                            <select name="teacher_id${fieldIndex}" id="teacher_id${fieldIndex}"
                                class="form-control @error('teacher_id${fieldIndex}') is-invalid @enderror teacher_id${fieldIndex}" onchange="getSubjectByTeacherEvent(event)">
                                <option value="">Select</option>
                                @if (count($staffs))
                                @foreach ($staffs as $staff)
                                <option value="{{ $staff->id }}">{{ $staff->first_name }} {{ $staff->last_name }}</option>
